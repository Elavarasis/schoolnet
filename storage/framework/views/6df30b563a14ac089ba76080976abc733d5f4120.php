...
<head>
  ...
  <?php echo Html::style('assets/admin/plugins/bootstrap-wysihtml5/bootstrap3-wysihtml5.min.css'); ?>
...
<!-- iCheck -->
<?php echo Html::script('assets/admin/plugins/iCheck/icheck.min.js');; ?>

<!-- Bootstrap WYSIHTML5 -->
<?php echo Html::script('assets/admin/plugins/bootstrap-wysihtml5/bootstrap3-wysihtml5.all.min.js');; ?>
  ...
  $(function () {
    ...
    $('body').on('click', '.select-all', function () {
        if ($(this).hasClass('allChecked')) {
            $('.select-single', allPages).prop('checked', false);
        } else {
            $('.select-single', allPages).prop('checked', true);
        }
    ...
    $(document).ready(function(){
		...
		$('#country_id_2').change(function(){
            ...
            $.ajax({
                ...
                success: function (resp) {
                    $.each(resp.states, function (key, value) {
                        $('#state_id_2').append('<option value="'+value.id+'">'+ value.name +'</option>');
                    });
                }
            });
        });
		
		$('#state_id_2').change(function(){
            var ramo, token, url, data;
            token = $('input[name=_token]').val();
            state_id = $('#state_id_2').val();
			url = '<?php echo url('/tenant/getcities'); ?>';
            data = {s_id: state_id};
            $('#city_id_2').empty();
            $.ajax({
                ...
                success: function (resp) {
                    $.each(resp.cities, function (key, value) {
                        $('#city_id_2').append('<option value="'+value.id+'">'+ value.city_name +'</option>');
                    });
                }
            });
        });
